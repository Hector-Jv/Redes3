...
def agregar_dispositivo(root, frame_extern):
    ...
    btn_ingresar = Button(frame, text="Ingresar", command=lambda: ingresar(root, frame, in_nombre, in_comunidad, in_version, in_puerto, in_ip))
    btn_ingresar.grid(column=0, row=6, ipadx=100, padx=15, pady=10, columnspan=2)
...
def ingresar(primary_frame, secondary_frame, nombre, comunidad, version, puerto, ip):

    with open('datos.json', 'r') as f:
        contenido_json = json.load(f)

    dispositivo = {"nombre": nombre.get(), "comunidad": comunidad.get(), "version": version.get(), "puerto": puerto.get(), "ip": ip.get()}
    contenido_json["dispositivos"].append(dispositivo)

    with open('datos.json', 'w') as f:
        json.dump(contenido_json, f, indent=4)

    agregar_dispositivo(primary_frame, secondary_frame)
